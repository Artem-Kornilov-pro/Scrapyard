...
Дашборд "Scrapyard Overview" подхватывается автоматически через provisioning (datasource закреплён за фиксированным `uid`, чтобы повторный provisioning никогда не создавал дубликат). Показывает частоту запросов по статусам, P95-латентность, латентность по эндпоинтам, метрики Redis, throughput задач Celery, статус воркера и количество задач по статусам.
...
> **Обновляете существующий `.env`?** Новые настройки со временем попадают в `.env.example` (ротация прокси, трейсинг и т.д.), но в ваш собственный `.env` автоматически не добавляются. Если после обновления какая-то функция как будто "молча" не работает — сравните файлы и перенесите недостающее:
> ```bash
